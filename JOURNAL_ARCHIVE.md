# Soft Box Project Journal — ARCHIVE

Older, settled entries moved out of `JOURNAL.md` to keep the active log small.
**Ordering: newest-LAST** (oldest at top, most recent at bottom) — the OPPOSITE of `JOURNAL.md`.
Entry content, headers, and dates are preserved verbatim.
...
# 2026-07-08 — CANONICAL COLLAPSE STAGE 3 (COLLAPSE COMPLETE): diagnostic-divergence visibility + param enforcement + tidy (canonical byte-identical)
Finishing stage — every diagnostic's model made VISIBLE, swept-param reclassification ENFORCED, safe orphaned dead
code swept, last lineage label mechanism-named; canonical default byte-identical throughout. **Tag `post-stage2-2026-07-08`
(`a10a871`) pushed** = rollback point. **PART 1 (diagnostic policy):** all 8 own-scene diagnostics (headTiltSweep/
boundGeom/forceDecomp/stiffnessAngleSweep/dCalib/catchSlipRecal/singleMolecule/swingKProbe) + canonDiag now emit a
centralized `diagMark` `[NON-CANONICAL DIAGNOSTIC]` banner — **all MARK, ZERO ROUTE**: each deliberately probes a
NON-canonical model (phase-2 config-1/perp motor, raw-law fine-dt single-motor, or bare kernel probe), so routing to
canonical buildScene would measure the wrong motor. diag/cycldiag/brakeDiagnose already run the canonical buildScene sc
⇒ no mark. Net: no diagnostic can silently pass as canonical. Byte-identical (labels). **PART 2 (D7 enforce):**
density/coltol get DENSITY_SET/COL_TOL_SET tracking + a LOUD `[SWEPT-PARAM WARNING]` on the main run when unset (chosen
over hard-error: run_gliding.sh's documented bare probe + several live scripts omit -density ⇒ hard-error breaks KEEP
paths). Byte-identical when passed (0 warning lines); diagnostics exempt. **PART 3 (orphaned sweep):** removed the 6
orphaned noise `scale*` methods from BrownianForceSystem (0-caller verified, gate byte-identical, canonical brownianForce
untouched). **CORRECTED the Stage-2 record — 4 methods Stage 2 called orphaned are LIVE in other harnesses** (implicitCorrect/
dashpotForces/setDashpot in V2OneXHarness; directedSwingHeadFrame in AxLockGateHarness) ⇒ KEPT (removal would BAIL). Left
scattered benign orphans (setBindMhat, catchSlipReleaseAvg/Recharge, cycleNoBoundAtp) documented. **PART 4 (label):**
CAP_ROW `faithfulRelease=`→`forcecapdetach=` (mechanism name; completes the -faithfulrelease→-forcecapdetach rename).
**PART 5:** `-tauavg` (feeds phase-2 dCalib/catchSlipRecal/singleMolecule) + `ATP_RELEASE`/`-noatprelease` (feeds opt-in
`-config1` cycleAtpDetach) RETAINED (feed KEEP paths; canonical reads neither ⇒ byte-identical) — cull only if jba culls
the dependents. **ACCEPTANCE (PART 6, no BAIL):** canonical default byte-identical to Stage 2 — CPU velFitX 4.583(800)/
3.268(6000), GPU 4.584/3.269; legacy KEEP 13.057 unchanged; MARK banner + SWEPT-PARAM warning both verified firing;
V2OneX/AxLockGate compile (kept methods intact). **THE CANONICAL COLLAPSE IS COMPLETE** (Stages 1–3: bake → delete/rename/
reclassify → visibility/enforce/tidy; canonical byte-identical throughout; rollback tags per stage). Report:
`CANONICAL_COLLAPSE_STAGE3.md`. **Deferred (each own task):** phase-2 two-point canonical motor (opt-in); `-forcecapdetach`
promotion; springs fine-dt recalibration; optional cosmetic sweep of remaining orphans + `-tauavg`/`ATP_RELEASE` disposition.
