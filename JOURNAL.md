...
## 2026-06-17 — Increment 6 — MINIMAL CONTRACTILE ASSAY DONE (the first genuinely contractile test). All 4 gates PASS GPU+CPU. A faithful ASSEMBLY of the validated structures + ONE new device kernel (`PinSystem`); NO new force law, NO new gather. New files only; all systems/stores reused byte-unchanged; production/`GlidingHarness` byte-unchanged; `BoA-v1ref` byte-clean. Report: `INC6_CONTRACTILE_ASSAY_FINDINGS.md`; spec: `INC6_CONTRACTILITY_ASSAY_SURVEY.md`.
**Scene (v2 adaptation of v1 `makeContractilityAssay`, flagged):** central bipolar minifilament backbone (+x, FIXED anchor) owning 16 dimers (8/end, 6b-splayed); two anti-parallel 8-seg filament chains in the +z up-head plane — filament A (plus +x, pinned +x) over the end2 up-head field, filament B (plus −x, pinned −x) over the end1 field. v1 `rodDotFil≥0` sorts polarity ⇒ end2 heads bind only A, end1 only B (both poles engage). v1 offsets filaments in Y w/ 3D splay; v2's 6b splay is x–z planar ⇒ both filaments in the +z plane is the faithful adaptation.
...
**4 gates:** (#1) crux above; (#4) no-motor control — pinned tips held EXACTLY (Δ0), steady tension relaxes to 3.3e-4 pN (chain transient decays, no load source); (#2) **IT CONTRACTS** — both poles engage (avgBound A=1.28 B=1.44 of 8 up-heads), force ON filament A=−5.7e-14 N (−x inward) / B=+1.13e-13 N (+x inward) = BOTH INWARD, anchor tension A=+0.057 B=+0.113 pN (both contractile), mean 0.085 pN = **261× the no-motor baseline**; heads stroke (200/100k steps); (#3) **CPU≡GPU** — (a) deterministic chain+PIN (no-motor, 600 steps) **bit-identical** float32 (coord Δ1.2e-7, pin end2 Δ1.2e-7, forceSum Δ5.5e-17) — validates the new `PinSystem` on device; (b) chaotic dynamic-bind aggregate-agree (bound 2=2, Lyapunov-decorrelated microstate).
**Weak-signal finding (SURFACED, not force-fit):** net tension ~0.1 pN, noise-dominated short-horizon; converges to clean bilateral contraction only over ≳40k steps (gate #2 uses 60k). Cause: only ~1.5/8 up-heads bound at once — duty-ratio-limited (a released head must rebind, NO fresh-motor reservoir unlike the gliding strip; rebind slow b/c the cocked rest angle 120° sits near the bind align gate). **Held-bound is intrinsically unstable on a pinned filament** (strain can't relax — both backbone & plus-end anchored, no translocation ⇒ forward-Euler diverges; physically correct, exactly why v1 needs catch-slip release) ⇒ dynamic binding is the only stable+faithful path. A strong sharp plateau needs more co-engaged heads (down-heads too / multiple minifilaments / 6c node) — the mechanism+readout are correct, magnitude scales with engaged-head count.
**v1 cross-check posture:** readout SET reproduced 1:1 (the viewer panel = v1 `ThreeJSWriter:262-277` schema: tensionA/B, anchorA/B, stats{boundHeads,peakBound,avg/ewma/peakTension,firstBind,…}, all from the ported defs). A tight numeric value match is **deliberately NOT attempted** (flagged): v2 geometry adapted ⇒ magnitudes not comparable; v1 contractility never experimentally calibrated (the §8 component-port-vs-emergent posture); forcing it = the force-fitting the spec forbids. Physics adjudicated by first principles (direction, sign, control, crux). jba's viewer eye is the final sign-off.
**New:** `PinSystem` (v1 `applyBenchmarkPins` port — position-snap, device-agnostic), `ContractileAssayHarness`, `run_contractile.sh`. Regression: 6a/6b/dimer-glide/mini-glide all rerun PASS. Next within inc 6: stronger engagement (down-head filaments / multiple minifilaments) + dynamic assembly/`myoMiniLifetime`; then 6c nodes (fresh v1 node snapshot per the recon settledness gate).
