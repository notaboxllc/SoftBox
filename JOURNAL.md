...
**Step-1→4 verification + matched v1 comparison (decisive cut):** (1) model verified — minifilament FULLY FREE (no pin/anchor/spring), ONLY filament plus-ends pinned, tension read at those pins, both poles engage; freeing it + 3D splay was the fix (~0.37→4.7 pN, ~13×). (2) readout panel complete (v1 schema). (3) **Step-3 force-coverage audit** (`-audit`): pinned-seg `forceSum` = chain + gather, residual **0** (frozen-pose decomposition), 0 motors on the pin ⇒ pin force is purely chain-transmitted; the v1 `jointForceSum`-omission gotcha CANNOT occur (v2 writes chain+gather into one `forceSum`). (4) **matched v1 (BoA-v1ref `/tmp` scratch, CPU 50k) vs v2 (50k), decomposed:** avgBound v1 5.38 / v2 ~6.4 (**comparable engagement**), avgTension v1 1.84 pN / v2 ~4.7 pN (**v2 higher, NOT low**), peak v1 3.32 / v2 ~24 pN. **Verdict: SHARED FAITHFUL PHYSICS** — v2 not `<` v1 on any channel (no bug); the one difference is **steadiness** (v1 steady symmetric plateau; v2 bursty/asymmetric, backbone drifts ~0.1 µm), localized to the **un-ported v1 confining chamber box** (`boxYDim 0.3/boxZDim 0.2` holds v1's free minifilament centered). A scene element, not a physics bug — **flagged, not auto-added** (planner-adjudicated; "fully free" was the ask, the box is environmental). v1 contractility itself uncalibrated (§8). Report `INC6_CONTRACTILE_ASSAY_FINDINGS.md` §6b/§7b.
Next within inc 6: **port v1's confining chamber box** (steady-state match to v1) + stronger engagement / dynamic assembly/`myoMiniLifetime`; then 6c nodes (fresh v1 node snapshot per the recon settledness gate).
