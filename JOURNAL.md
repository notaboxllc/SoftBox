...
discrete-site / linear-ramp motor. **No powered gliding or twirling screen has been run; no viscosity
sensitivity is claimed.** Report: `docs/VISCOSITY_SENSITIVITY_FINDINGS.md` **Part II** (Part I, the
2026-07-09 three-point probe, is preserved unaltered as historical context).

**What was learned.**
- **There was no coherent viscosity path, and none at all in the current assay.** `Constants.aeta = 0.1`
  is a compile-time `static final` (javac inlines it ⇒ no runtime override is possible); `ChiralSiteHarness`
  exposed no viscosity lever. The `-aeta` flags that exist elsewhere scale the **filament drag only** —
  a partial diagnostic rescale. **Part I therefore does not transfer to the current motor.**
- **`-eta` (default-off) is DATA-ONLY** — the `applyS2Lawn` precedent. Only 3 of the 17 per-motor params
  are viscosity-bearing (`gammaPhi[8]`, `gammaPsi[9]`, `g4gammaNode[16]`); plus filament and motor-body
  drag tensors. No kernel edit, no buffer resize, no TaskGraph change. Stiffnesses untouched.
- **FDT is preserved by construction:** all four Brownian channels build their amplitude as
  `sqrt(2kT·γ/dt)` from the same γ that is scaled ⇒ `D = kT/γ ∝ 1/η` with no hand-scaled noise.
- **STRUCTURAL: two force-law families.** Class I (S2 beam, φ/ψ, head roll, filament rigid body) relaxes
  ∝ η. Class II — the **fracMove** family (chain PAIRS F3/F4, F10 alignment) — relaxes a **fixed fraction
  per STEP**, γ-independent. Under **fixed dt** Class II does not respond to viscosity at all (a different
  physical system, not merely under-resolved); under `dt(η) = dt₀·η/η₀` it relaxes at `k/dt ∝ 1/η` and
  matches Class I. **The scaled-dt protocol is required for coherence, not hygiene.**
- **Gates.** Stage 0: all 8 drag channels exactly ∝ η, stiffnesses invariant, η=0.1 an exact no-op.
  Stage 1: dimensionless factors **exactly invariant** across η 0.10→0.01 (0.52011 / 3.4561 / 111.41 /
  0.19099) ⇒ no new numerical regime; and the per-step Brownian displacement is **invariant** (it would
  grow ×3.16 at fixed dt). Stage 2: ζ, γ_roll ∝ η; D ∝ 1/η; τ_S2 ∝ η — all exact.
- **η = 0.01 adjudicated trustworthy.** The twirl equivalence (where head roll is the primary observable)
  **PASSES at η=0.01 on a longer horizon and cleaner than at η=0.10**. The gliding equivalence trips only
  the **absolute** `max|dOmega| < 1e-5` gate on a diagnostic accumulator (rel 3.34e-06 vs 1.31e-06 ≈ 29 vs
  11 float32 ULP) while **every decision channel is exact** (bind/site/azimuth 0 mismatch).
  **The gate was NOT relaxed to make it pass** — recorded as a bounded, decision-exact anomaly.

**What's open.** Stages 3–5: the 8-seed gliding and twirling viscosity screens at η = 0.10/0.05/0.02/0.01
with scaled dt and both ε signs, plus the fixed-dt vs scaled-dt discrimination. Sized at ~5–6 h GPU
device-resident wall-clock (matched 20 ms physical duration ⇒ 18× the η=0.1 step count summed over the
four arms). Canonical viscosity **unchanged**; nothing default-on.
