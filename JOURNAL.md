...
## 2026-06-17 — Increment 6 — MINIMAL CONTRACTILE ASSAY DONE (the first genuinely contractile test). All 4 gates PASS GPU+CPU. A faithful ASSEMBLY of the validated structures + two new device kernels (`PinSystem` + `BackboneAnchorSystem`); NO new force law, NO new gather. New files only; all systems/stores reused byte-unchanged; production/`GlidingHarness` byte-unchanged; `BoA-v1ref` byte-clean. Report: `INC6_CONTRACTILE_ASSAY_FINDINGS.md`; spec: `INC6_CONTRACTILITY_ASSAY_SURVEY.md`. (Reworked per jba to the v1-faithful geometry: offset filaments + free Brownian minifilament + thermal search.)
**Scene (faithful to v1 `makeContractilityAssay`):** central bipolar minifilament backbone (+x) owning 16 dimers (8/end), **FREE + undergoing Brownian motion** (the v1 thermal search — per jba), softly centered by `BackboneAnchorSystem` (the v1 confining-box stand-in); two anti-parallel 8-seg filament chains **offset in Y at ±0.05 µm** (v1 `contractFilYOffset`) — filament A (plus +x, pinned +x) at +Y, filament B (plus −x, pinned −x) at −Y, straddling the minifilament. Dimers splay in the x–Y plane so heads project ±Y; v1 `rodDotFil≥0` sorts polarity ⇒ end2 heads bind only A, end1 only B (both poles engage).
**Brownian thermal search = the binding enabler (jba's key point, and v1's):** v1's dimer rods are AXIAL (`makeMyosinDimers` — radial offset commented out) ⇒ a head tip reaches only ~(lever+head)≈28 nm perpendicular while filaments sit at 50 nm; the gap is bridged by the bind capture radius + the **Brownian wiggle of rods/heads/backbone** ("thermal search is the essential enabler"). With Brownian on, engagement ~1.5→2.3/pole and the signal is ~5× stronger + symmetric. A fully free backbone diffused ~185 nm off-centre (>its 180 nm length) ⇒ asymmetric; the soft centering (`BackboneAnchorSystem`, k≈3e-4 N/m) keeps it "in the middle" (excursion ~16 nm) while it still wiggles (~3–13 nm).
...
**4 gates:** (#1) crux above; (#4) no-motor control — pinned tips held EXACTLY (Δ0), steady tension relaxes to 3.3e-4 pN (chain transient decays, no load source); (#2) **IT CONTRACTS** — both poles engage (avgBound ~2.3/pole of 8 up-heads), force ON filament A=−x / B=+x = BOTH INWARD, anchor tension A≈+0.27 B≈+0.46 pN (both contractile, SYMMETRIC), mean ~0.37 pN = **~1100× the no-motor baseline**, converges by ~20k steps; (#3) **CPU≡GPU** — (a) deterministic chain+PIN (no-motor, 600 steps) **bit-identical** float32 (coord Δ1.2e-7, pin end2 Δ1.2e-7, forceSum Δ5.4e-17) — validates the new `PinSystem` on device; (b) chaotic full-Brownian path (incl. `brownMot`/`brownBb`/`center`) aggregate-agree (bound 4=4, Lyapunov-decorrelated microstate).
**Signal/held-bound finding (SURFACED):** with the Brownian search the contraction is symmetric + ~5× stronger than the earlier deterministic placement; still a single small minifilament (no fresh-motor reservoir) ⇒ avgBound duty-ratio-limited (~2.3/8 up-heads). **Held-bound is intrinsically unstable on a pinned filament** (strain can't relax — backbone softly anchored + plus-end pinned, no translocation ⇒ forward-Euler diverges; physically correct, exactly why v1 needs catch-slip release) ⇒ dynamic binding is the only stable+faithful path. A stronger sharp plateau scales with co-engaged head count (down-head filaments / multiple minifilaments / 6c node).
**v1 cross-check posture:** readout SET reproduced 1:1 (the viewer panel = v1 `ThreeJSWriter:262-277` schema). A tight numeric value match is **deliberately NOT attempted** (flagged): v1 contractility never experimentally calibrated (the §8 component-port-vs-emergent posture); forcing it = force-fitting. Physics adjudicated by first principles (direction, sign, control, crux). jba's viewer eye is the final sign-off.
**Fidelity:** filaments offset ±0.05 µm (v1 exact), free Brownian minifilament (v1), axial dimer rods (v1), thermal search (v1). ONE flagged adaptation: dimer splay is x–Y-planar (heads project ±Y toward the two straddling filaments) vs v1's 3D azimuthal splay; the soft backbone centering stands in for v1's confining box.
**New:** `PinSystem` (v1 `applyBenchmarkPins` port), `BackboneAnchorSystem` (v1 confining-box stand-in), `ContractileAssayHarness`, `run_contractile.sh`. Regression: 6a/6b/dimer-glide/mini-glide all rerun PASS. Next within inc 6: stronger engagement (down-head filaments / multiple minifilaments) + dynamic assembly/`myoMiniLifetime`; then 6c nodes (fresh v1 node snapshot per the recon settledness gate).
